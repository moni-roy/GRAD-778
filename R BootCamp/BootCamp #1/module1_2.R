...
data.txt.df <- read.table("comm_data.txt", header=T, sep="") 
new_df <- data.txt.df[, c("Hab_class", "C_DWN", "C_UPS")]
colnames(new_df) <- c("Class","Downstream", "Upstream")

write.table(new_df, file="comm_data_export.txt", sep=" ", col.names=TRUE, row.names=FALSE)
...
turtles.NArm.df <-na.omit(turtles.df)

write.table(turtles.NArm.df, file = "turtle_NA_removed_data.txt", col.names = T, row.names = F, sep = ",")
...
# male_turtles <- turtles.df[, which(turtles.df$sex=="male")]  # uncomment this to run it...

# the given comman will not work. As the 'which()' function will return the row numbers. So, we need to change the command by switching the which function to row position. the corrected command is given below.
male_turtles <- turtles.df[which(turtles.df$sex=="male"), ] 
